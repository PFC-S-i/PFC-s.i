...
function normalizeCoins(raw: unknown): CoinOption[] {
  if (!Array.isArray(raw)) return [];

  return (raw as CoinLike[])
    .map((c) => {
    ...
    .filter((c): c is CoinOption => c !== null);
}

const TOP_TTL_MS = 2 * 60 * 1000;
...
export async function topCoins(
  ...
  const run = async (): Promise<CoinOption[]> => {
    ...
    try {
      const data = await apiFetch<CoinLike[]>(url, { signal });
...
export async function searchCoins(
  ...
  const url = `/api/prices/coins/search?q=${encodeURIComponent(
    term
  )}&per_page=${perPage}`;
  ...
  try {
    ...
    if (e instanceof RateLimitError) {
      const base =
        (_topCache?.data?.length ? _topCache.data : lsRead()?.data) ??
        COMMON_COINS;
...
export async function getCoinChart(
  ...
  let r = await fetch(
    `/api/prices/coin-chart/${encodeURIComponent(id)}?hours=${hours}&vs=${vs}`,
    {
      method: "GET",
      cache: "no-store",
    }
  ...
  if (r.ok) {
    const raw = (await r.json()) as {
      items?: { t: number; price: number }[];
      prices?: [number, number][];
    };
    if (raw.items)
      return raw.items.map((p) => ({ t: Number(p.t), price: Number(p.price) }));
    const arr = raw.prices ?? [];
    return arr.map((p) => ({ t: Number(p[0]), price: Number(p[1]) }));
  }

  r = await fetch(
    `/api/prices/coins/${encodeURIComponent(id)}/chart?hours=${hours}&vs=${vs}`,
    {
      method: "GET",
      cache: "no-store",
    }
