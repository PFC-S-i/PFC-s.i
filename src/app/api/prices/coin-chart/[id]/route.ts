...
async function fetchWithRetry(url: URL, headers: Record<string, string>, tries = 2) {
  for (let i = 0; i <= tries; i++) {
    const r = await fetch(url.toString(), { headers, next: { revalidate } });
    if (r.status !== 429) return r;
    const retryAfter = Number(r.headers.get("retry-after") ?? "2");
    await new Promise((res) => setTimeout(res, clamp(retryAfter * 1000, 1000, 5000)));
  }
...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!r.ok) {
    ...
    return new NextResponse(
      JSON.stringify({ error: "coingecko_error", status: r.status, details: text.slice(0, 300) }),
  ...
  const items = prices.map(([t, price]) => ({ t: Number(t), price: Number(price) }));

  const res = NextResponse.json({ items });
  res.headers.set("Cache-Control", `s-maxage=${revalidate}, stale-while-revalidate=3600`);
  return res;
}
