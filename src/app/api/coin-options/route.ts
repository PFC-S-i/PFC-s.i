...
async function fetchWithRetry(url: URL, headers: Record<string, string>, tries = 2): Promise<Response> {
  for (let i = 0; i <= tries; i++) {
    const r = await fetch(url.toString(), { headers, next: { revalidate: 3600 } });
    if (r.status !== 429) return r;
    const retryAfter = Number(r.headers.get("retry-after") ?? "2");
    await new Promise((res) => setTimeout(res, Math.min(5000, Math.max(1000, retryAfter * 1000))));
  }
...
export async function GET(req: NextRequest) {
  ...
  const count = Math.min(250, Math.max(1, Number(searchParams.get("count") ?? "20")));
  ...
  const pending = (async () => {
    ...
    if (!r.ok) {
      if (hit) {
        ...
        staleRes.headers.set("Cache-Control", "s-maxage=60, stale-while-revalidate=86400");
        return { data: hit.data, exp: now + 60 * 1000 } as CacheEntry;
      }
      const text = await r.text();
      throw new Response(
        JSON.stringify({ error: "coingecko_error", status: r.status, details: text.slice(0, 200) }),
        { status: 502, headers: { "content-type": "application/json" } }
      );
    }

    const raw = (await r.json()) as unknown;
    const items: CGMarketsItem[] = Array.isArray(raw) ? raw.filter(isCGMarketsItem) : [];
    const data: CoinRow[] = items.map((d) => ({ id: d.id, name: d.name, symbol: d.symbol ?? null }));
  ...
  try {
    ...
    return new NextResponse(JSON.stringify({ error: "unexpected_error", details: String(e?.message ?? e) }), {
      status: 500,
